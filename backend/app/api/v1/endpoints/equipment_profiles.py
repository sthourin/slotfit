...
async def list_equipment_profiles(
    ...
    query = select(EquipmentProfile).offset(skip).limit(limit).order_by(
...
async def get_equipment_profile(
    ...
    query = select(EquipmentProfile).where(EquipmentProfile.id == profile_id)
...
async def create_equipment_profile(
    ...
    if profile_data.is_default:
        await db.execute(
            ...
            .where(EquipmentProfile.is_default == True)
...
async def set_default_equipment_profile(
    ...
    """Set an equipment profile as the default (clears other defaults)"""
    query = select(EquipmentProfile).where(EquipmentProfile.id == profile_id)
    result = await db.execute(query)
    profile = result.scalar_one_or_none()
    ...
    # Clear is_default on all other profiles
    await db.execute(
        update(EquipmentProfile)
