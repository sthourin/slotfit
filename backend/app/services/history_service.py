...
async def exercise_set_history(
    ...
    Returns:
        [{"performed_at": datetime,
          "sets": [(weight, reps, time_seconds, distance_meters), ...]}]

    Both generations now carry duration and distance. `workout_sets` gained the
    columns so three years of imported ergometer and carry work is visible to
    progression - a conditioning feature whose history starts empty can never
    prescribe a pace.
    """
    legacy_query = (
        select(
            WorkoutSession.id, WorkoutSession.completed_at,
            WorkoutSet.weight, WorkoutSet.reps, WorkoutSet.time_seconds,
            WorkoutSet.distance_meters, WorkoutSet.set_number,
        )
    ...
    new_query = (
        select(
            ...
            EntrySet.weight, EntrySet.reps, EntrySet.time_seconds,
            EntrySet.distance_meters, EntrySet.set_number,
        )
    ...
    # (set_number, weight, reps, time_seconds, distance_meters)
    sets_by_session: dict[
        tuple[str, int],
        list[tuple[int, float | None, int | None, int | None, float | None]],
    ] = defaultdict(list)
    completed_at_by_session: dict[tuple[str, int], datetime] = {}

    for session_id, completed_at, weight, reps, time_seconds, distance, set_number in legacy_rows:
        if completed_at is None:
            continue
        key = ("legacy", session_id)
        sets_by_session[key].append((set_number, weight, reps, time_seconds, distance))
        completed_at_by_session[key] = completed_at

    for session_id, completed_at, weight, reps, time_seconds, distance, set_number in new_rows:
        if completed_at is None:
            continue
        key = ("new", session_id)
        sets_by_session[key].append((set_number, weight, reps, time_seconds, distance))
    ...
    for key in ordered_keys:
        ...
        performances.append({
            ...
            "sets": [
                (weight, reps, time_seconds, distance)
                for _n, weight, reps, time_seconds, distance in ordered_sets
            ],
        })
    return performances
